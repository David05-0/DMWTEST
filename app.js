...
function setupApp() {
  ...
  if (currentRole === 'admin') {
    nav.innerHTML = `
      ...
      <div class="nav-item" onclick="navigate('page-requests')"><span class="icon">📋</span> All Requests <span class="nav-badge" id="pending-badge">0</span></div>
    `;
  } else {
    nav.innerHTML = `
  ...
  const firstPage = currentRole === 'admin' ? 'page-dashboard' : 'page-new-request';
  navigate(firstPage);
}
...
function navigate(pageId) {
  ...
  if (pageId === 'page-new-request') {
    resetRequestForm();
    actions.innerHTML = `<span style="font-size:13px;color:var(--gray-400);">Form Appendix 63 — RIS</span>`;
  }
}
...
function renderDashboard() {
  ...
  const totalItems = SUPPLIES.reduce((s,i)=>s+i.available,0);

  document.getElementById('dash-stats').innerHTML = `
    <div class="stat-card"><div class="stat-label">Total Supply Items</div><div class="stat-value">${SUPPLIES.length}</div><div class="stat-sub">${totalItems} total units available</div></div>
    <div class="stat-card teal"><div class="stat-label">Total Requests</div><div class="stat-value">${REQUESTS.length}</div><div class="stat-sub">All time submissions</div></div>
    <div class="stat-card" style="border-left-color:#e8a020"><div class="stat-label">Pending</div><div class="stat-value">${pending}</div><div class="stat-sub">Awaiting action</div></div>
    <div class="stat-card green"><div class="stat-label">Approved / Issued</div><div class="stat-value">${approved+issued}</div><div class="stat-sub">Processed requests</div></div>
  `;

  const recent = [...REQUESTS].reverse().slice(0,5);
  const tb = document.getElementById('dash-recent-table');
  if (!recent.length) {
    tb.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="icon">📭</div><p>No requests yet</p></div></td></tr>`;
  } else {
  ...
  tb.innerHTML = [...data].reverse().map(r => `
    <tr>
      ...
      <td>
        ...
        ${r.status==='Pending' ? `
          <button class="btn btn-success btn-sm" onclick="updateStatus('${r.risNo}','Approved')" style="margin-left:4px">✓ Approve</button>
          <button class="btn btn-danger btn-sm" onclick="updateStatus('${r.risNo}','Rejected')" style="margin-left:4px">✕ Reject</button>
        ` : ''}
        ${r.status==='Approved' ? `
...
function updateStatus(risNo, status) {
...
function viewRIS(risNo, isAdmin) {
  ...
  const itemsHtml = r.items.map(i => `
    <tr>
      ...
      <td><span class="badge ${i.qty <= (SUPPLIES.find(s=>s.id===i.id)?.available||0) ? 'badge-approved' : 'badge-rejected'}">${i.qty <= (SUPPLIES.find(s=>s.id===i.id)?.available||0) ? 'Available' : 'Check Stock'}</span></td>
    </tr>
  `).join('');
  ...
  document.getElementById('modal-ris-body').innerHTML = `
    ...
    ${r.remarks ? `<div style="margin-top:16px;padding:12px;background:var(--gray-50);border-radius:8px;font-size:13px;color:var(--gray-600);">Remarks: ${r.remarks}</div>` : ''}
  `;
  ...
  if (isAdmin && r.status === 'Pending') {
    footer.innerHTML = `
      ...
      <button class="btn btn-danger" onclick="updateStatus('${r.risNo}','Rejected');closeModal('modal-ris')">✕ Reject</button>
      <button class="btn btn-success" onclick="updateStatus('${r.risNo}','Approved');closeModal('modal-ris')">✓ Approve</button>
    `;
  } else if (isAdmin && r.status === 'Approved') {
    footer.innerHTML = `
      <button class="btn btn-outline btn-sm" onclick="openPrintRIS('${r.risNo}')">🖨️ Preview RIS</button>
      <button class="btn btn-primary" onclick="updateStatus('${r.risNo}','Issued');closeModal('modal-ris')">Mark as Issued</button>
    `;
  } else if (isAdmin) {
    footer.innerHTML = `
...
// ────────────────────────────────────────────────────────────
// PRINT RIS — Official Appendix 63 Form
// ────────────────────────────────────────────────────────────
let currentPrintRIS = null;

function buildRISFormHTML(r) {
  // Fill rows: minimum 15 rows for the table
  const BLANK_ROWS = 15;
  const rows = [...r.items];
  ...
  const rowsHtml = rows.map((item, idx) => {
    ...
    return `<tr>
      ...
      <td>${stockYes}</td>
      <td>${stockNo}</td>
      <td>${r.status === 'Issued' ? item.qty : (r.status === 'Approved' ? item.qty : '')}</td>
      <td></td>
    </tr>`;
  }).join('');
  ...
  <div class="ris-form" id="ris-form-content">
    <div class="ris-top-notes">
      3 copies<br>
      1 for Accountant (RSMI)<br>
      1 for Requisitioning Unit<br>
...
function printRIS() {
  ...
  const area = document.getElementById('print-ris-area');
  area.innerHTML = buildRISFormHTML(currentPrintRIS);
  window.print();
}
...
function renderSupplyPicker() {
  ...
  grid.innerHTML = data.map(s => {
    ...
    return `
      <div class="supply-item${inCart?' selected':''}${s.available===0?' out-of-stock':''}" onclick="toggleSupply(${s.id})" id="si-${s.id}">
...
function toggleSupply(id) {
  ...
  if (cart[id]) {
    ...
    cart[id] = { id, name: s.name, unit: s.unit, qty: 1, max: s.available };
  }
  renderSupplyPicker();
  renderCart();
}
...
function updateCartQty(id, val) {
  if (!cart[id]) return;
  const n = Math.max(1, Math.min(parseInt(val)||1, cart[id].max));
  cart[id].qty = n;
}
...
function submitRequest() {
  ...
  const risNo = `RIS-2026-${String(risCounter++).padStart(4,'0')}`;
